...
class zpl:
    ...
    def save_printer_json(
        self, json_filename="/etc/printer_config.json", relative=True
    ):
        """
    ...
    def formulate_zpl(
        ...
        if not os.path.exists(zpl_file):
            zpl_file = (
                str(files("zebra_day"))
                + f"/etc/label_styles/tmps/{label_zpl_style}.zpl"
            )
            if not os.path.exists(zpl_file):
                raise Exception(
    ...
    def generate_label_png(self, zpl_string=None, png_fn=None, relative=False):
        ...
        if response.status_code == 200:
            ...
            print(
                f"Failed to convert ZPL to image. Status code: {response.status_code}",
                file=sys.stderr,
            )
    ...
    def print_zpl(
        ...
        if label_zpl_style in [None, "", "None"]:
            label_zpl_style = self.printers["labs"][lab][printer_name][
                "label_zpl_styles"
            ][
                0
            ]  # If a style is not specified, assume the first
        elif (
            label_zpl_style
            not in self.printers["labs"][lab][printer_name]["label_zpl_styles"]
        ):
            print(
        ...
        os.system(
            f"echo '{lab}\t{printer_name}\t{uid_barcode}\t{label_zpl_style}\t{printer_ip}\t{print_n}\t{client_ip}\t{zpl_content}\n' >> {str(files('zebra_day'))}/logs/print_requests.log"
        )
        ...
        if printer_ip in ["dl_png"]:
            png_fn = (
                str(files("zebra_day"))
                + f"/files/zpl_label_{label_zpl_style}_{rec_date}.png"
            )
...
def zday_start():
    ...
    os.system(
        f"python {str(files('zebra_day'))}/bin/zserve.py "
        + os.path.dirname(zdpm.__file__)
    )
